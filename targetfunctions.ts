...
let motion: number=0; //variable to set the motion
let myTimer: number;
let speed = 0.01;
interface Square {
    centerX: number;
    centerY: number;
    dx: number;  // direction in x-axis
    dy: number;  // direction in y-axis
}
...
function moveTarget(){
    ...
    let allSquarePoints: vec4[] = [];  // Array to keep updated square points
...
function makeSquaresAndBuffer(numTargets: number) {
    ...
    let bufferId:WebGLBuffer = gl.createBuffer(); //create a new buffer object and store its ID in bufferId
    gl.bindBuffer(gl.ARRAY_BUFFER, bufferId);
...
function checkHit(event: MouseEvent) {
    let rect: ClientRect = canvas.getBoundingClientRect();
    let canvasX: number = (event.clientX - rect.left) / canvas.clientWidth * 2 - 1;
    let canvasY: number = 1 - (event.clientY - rect.top) / canvas.clientHeight * 2;
...
function showFeedback(message: string) {
    ...
    const feedbackElem = document.getElementById("feedback") as HTMLParagraphElement;
